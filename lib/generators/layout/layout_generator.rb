## Generator for the base of an app. Install several things

class LayoutGenerator < Rails::Generators::Base
  source_root File.expand_path('../templates', __FILE__)
  argument :layout_name, :type => :string, :default => "application"
   
  # Add the bootstrap files and the minimum CSS 
  def add_bootstrap
  ...
  # Add the basic images
  def add_images
    directory 'images', 'app/assets/images'
  end
  
  # Add the default methods for the models
  def add_common_concerns
    directory 'concerns', 'app/models/concerns'
  end
  
  # Add the new config development to send email in development through our gmail address for development
  def add_config_development
    copy_file "development.rb", "config/environments/development.rb"
    copy_file "secrets.yml", "config/secrets.yml"
  end
  
  # Add the plugins folder for the project. Automatic load
  def add_plugins_to_vendor
    directory 'plugins', 'vendor/assets/stylesheets/plugins'
    directory 'plugins', 'vendor/assets/javascripts/plugins'
  end
  
  # Change the gemfile with the useful gems
  def change_gemfile
    copy_file "Gemfile", "Gemfile"
  end
  
  # Change the database yml for PG
  def change_database_yml
    template "database.yml.erb", "config/database.yml"
  end
  
  # Unnicorn configuration
  def change_unicorn
  ...
  # Change in order to take into account translation. DO NOT WORK
  ...
  def add_common_commands_textfile
    copy_file "common_commands", "common_commands"
  end
  
  # Create database and the compiled assets
  def rake_commands
    puts "You should change the name of the database in database.yml to avoid conflict in the development mode"
    #rake 'db:create'
    #rake 'db:migrate'
  end
